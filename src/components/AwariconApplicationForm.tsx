...
export default function AwariconApplicationForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const omega = computeOmega(form.operatingYears);
      const integrity = computeIntegrity(form.integrityFlags);
      const computed = computeScore(form.phi, form.df, omega, integrity);
      const recommendedTier = determineRecommendedTier(computed);
      const generatedId = `aw-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
      ...
      const localResult: SubmitResult = {
        ...
        recommendedTier,
        status: 'submitted',
        message: 'Application recorded and queued for verification.'
      };

      try {
        const existing = JSON.parse(localStorage.getItem('awaricon_applications') || '[]');
        existing.unshift({ ...form, ...localResult, submittedAt: new Date().toISOString() });
        localStorage.setItem('awaricon_applications', JSON.stringify(existing));
      } catch {
        // storage fallback
      }
