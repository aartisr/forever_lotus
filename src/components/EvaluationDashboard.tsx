...
const PrincipleCard: React.FC<{
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-sm p-5 hover:border-lotus-gold/40 transition-all duration-300">
    ...
    <div className="min-h-screen bg-gradient-to-b from-lotus-bg via-[#f5f0e8]/30 to-lotus-bg px-5 py-12 sm:px-8">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        ...
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-950/20 p-6">
          <h3 className="font-serif text-xl font-bold text-emerald-300 mb-4">{t.strengths}</h3>
            ...
            {evaluation.summary.strengths.length > 0 ? (
              evaluation.summary.strengths.map((strength, i) => (
                <li key={i} className="flex items-start gap-3 text-emerald-200">
                  <span className="text-xl mt-0.5 text-emerald-400">✓</span>
        ...
        <div className="rounded-2xl border border-amber-500/30 bg-amber-950/20 p-6">
          <h3 className="font-serif text-xl font-bold text-amber-300 mb-4">{t.opportunities}</h3>
          <ul className="space-y-3">
            {evaluation.summary.opportunities.length > 0 ? (
              evaluation.summary.opportunities.map((opp, i) => (
                <li key={i} className="flex items-start gap-3 text-amber-200">
                  <span className="text-xl mt-0.5 text-amber-400">→</span>
      ...
      {evaluation.summary.critical_actions.length > 0 && (
        <div className="max-w-6xl mx-auto mb-12 rounded-2xl border-2 border-lotus-gold/40 bg-lotus-gold/10 p-6">
          ...
          <ol className="space-y-2">
            {evaluation.summary.critical_actions.map((action, i) => (
              <li key={i} className="flex items-start gap-3 text-lotus-cream">
                <span className="font-bold min-w-fit text-lotus-gold">{i + 1}.</span>
        ...
        <div className="space-y-3">
          {evaluation.submission_checklist_mapping.map((item, idx) => {
            const statusColors: Record<string, { bg: string; text: string; label: string }> = {
              met: { bg: 'rgba(16, 185, 129, 0.2)', text: '#34d399', label: t.met },
              partial: { bg: 'rgba(245, 158, 11, 0.2)', text: '#fbbf24', label: t.partial },
              unmet: { bg: 'rgba(239, 68, 68, 0.2)', text: '#f87171', label: t.unmet },
              'not-applicable': { bg: 'rgba(156, 163, 175, 0.2)', text: '#9ca3af', label: t.not_applicable },
            };
            ...
            return (
              <div key={idx} className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
                ...
                {item.recommendations.length > 0 && (
                  <div className="bg-black/40 border border-white/5 rounded-lg p-3 mt-2">
